<template>
  <div class="w-full overflow-x-hidden">
    ...
    <div v-for="(genre, index) in posts" :key="index"
      class="my-4 group"
      >
      <div class="flex items-center justify-between md:justify-start gap-4 mx-5 overflow-hidden mb-1" v-if="!isLoader" :class="genre.size_type == 'promo' ? 'hidden' : ''">
      ...
      <template v-if="genre?.content">
        <content-card :styleClass="getStyle(genre.size_type)"
          ...
          :size="genre.size_type"
          :setHeight="setHeight(genre.size_type)"
          :visibleNumber="getNumber(genre.size_type)"
          :contents="genre.content" />
          <promo-content v-else :contents="genre.content" :styleClass="getStyle(genre.size_type)" :genre="genre" />
      </template>
    </div>
