<template>
  ...
  <div class="hidden md:flex flex-col p-1" v-if="comments?.length > 0">
    ...
    <Divider />
    <div class="flex items-start justify-start flex-col mb-4 group" v-for="(item, index) in comments" :key="index">
      <div class="flex items-center gap-3">
        <img :src="item.user?.image" :alt="item.user.name[0]" class="rounded-full w-[30px] h-[30px] bg-zinc-400 flex items-center justify-center text-center text-2xl m-0 p-0">
        <p class="font-fold text-xl">@{{ item.user.name }}</p>

        <Icon icon="tabler:trash-x-filled" v-if="item.user.id == user.id" width="24" class="hidden group-hover:block hover:text-red-500 cursor-pointer m-0 p-0" height="24" v-tooltip="'Delete'" placeholder="right" @click="removeComment(content?.id, item.id)" />
      </div>
