<template>
  <div class="w-full h-auto mb-2 md:pb-[80px]">
    <div class="h-[320px] md:h-[720px] bg-slate-800 flex items-center justify-center flex-col gap-4"
      v-if="content?.premium === '1' && subscription === '0'">
    ...
    <content-title />
    <content-details :contentID="content?.id" />
    <comment-post />
    <comment-view />
  </div>
</template>
...
export default {
  ...
  updated() {
    if (this.content?.premium !== "1" || this.subscription !== "0") {
      if (this.isPlayer && this.content?.content_url) {
        this.isPlayer.new(this.content?.content_url);
      }
      if (this.content?.content_url && this.isPlayer === false) {
        this.isPlayer = Player({
          id: 'player',
          src: this.content?.content_url,
      ...
      this.getComment(this.content?.id);
    } else {
      this.isPlayer = false;
    }
