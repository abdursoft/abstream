...
export default function absVideo(premium) {
  if (typeof premium === 'object') {
    const def = {
      ...
      fn: {
        ...
        fontColor: function setColor(element, fill = false) {
          ...
          fill === false ? (element.style.color = colors) : (element.style.fill = colors)
        },
        hoverColor: function hover(element, fill = false, mute = false) {
          ...
          element.addEventListener('mouseover', () => {
            fill === false ? (element.style.color = hColors) : (element.style.fill = hColors)
          })
          element.addEventListener('mouseleave', () => {
            fill === false ? def.fn.fontColor(element, false) : def.fn.fontColor(element, true)
            def.fn.activeColor()
          })
        ...
        setPopover: function myPopover(item) {
          ...
          premium.tooltip !== false ? def.fn.getObject(item).appendChild(popover) : true
        },
        ...
        getAnalytics: () => {
          if (premium.videoAnalytics && typeof premium.videoAnalytics === 'object') {
    ...
    def.fn.css(controls, {
      background:
        premium?.controls?.background ?? 'linear-gradient(to top, rgba(0, 0, 0, 0.7), transparent)',
      width: '100%',
      height: '50px',
      position: 'absolute',
      left: 0,
    ...
    title.textContent = premium?.title ?? 'Title gose to here';
    ...
    def.fn.css(divider, def.fn.getCss(premium?.durationArea?.divider?.css, 'dividerCss'))
    ...
    def.fn.css(speedControl, def.fn.getCss(premium?.volumeControl, 'volumeControl'))
    ...
    def.fn.css(speedPlacement, def.fn.getCss(premium?.playback?.placement?.css, 'speedPlacement'))
    def.fn.html(
      speedPlacement,
    ...
    def.fn.css(episodeContainer, def.fn.getCss(premium?.episodeContainer, 'episodeContainer'))
    ...
    def.fn.css(playlistControl, def.fn.getCss(premium?.playlistControl, 'playlistControl'))
    ...
    def.fn.css(screenControl, def.fn.getCss(premium?.screenControl, 'screenControl'))
    ...
    speedPlacement.addEventListener('click', () => {
      if (isSpeedMenu) {
        ...
        def.fn.css(speedArea, def.fn.getCss(premium?.playback?.placement?.speedArea, 'speedArea'))
        setPlayback()
      }
    ...
    function initVolumeSlider() {
      if (volumeSlider != undefined) {
        ...
        def.fn.css(volumeSliderArea, def.fn.getCss(premium?.volumeSliderArea, 'volumeSliderArea'))
        def.fn.css(volumeContainer, def.fn.getCss(premium?.volumeContainer?.css, 'volumeContainer'))
      }
    ...
    itemLoop.addEventListener('click', () => {
      ...
      for (let l = 0; l < loops.length; l++) {
        loopItem += `<absdiv class="abs_loop ${
          ...
          loops[l]
        }'>${loops[l]}</p><p style='font-size:12px;font-family:sans-serif;margin:0 !important;'>${
    ...
    if (device.os !== 'Android' && device.os !== 'Ios' && device.os !== 'Ipad') {
      hoverItems.forEach((item) => {
        item.addEventListener('mouseover', () => {
          ...
          item.style.background = premium?.iconHoverColor ?? 'rgba(219, 30, 30, 0.88)'
          ...
          for (var i = 0; i < video.textTracks.length; i++) {
            if (video.textTracks[i].kind === kind) {
              if (video.textTracks[i].language === value) {
                ...
                googleEvent('subtitle_change', 'Video subtitle', video.textTracks[i])?.label
              }
            }
          }
        }
    ...
    function playNextItem() {
      ...
      if (urls.length > playIndex) {
        playIndex++
      } else {
        playIndex = urls.length
      }
    ...
    function launchIntoFullscreen(element) {
      if (element.requestFullscreen) {
        ...
        console.log(`Fullscreen API is not supported in ${device.browser} browser.`)
      }
      def.fn.html(screenControl, def.icons.fullScreenOut)
      def.fn.togglePopover(screenControl, 'exitFullscreen')
      if (device.os !== 'iOS' && device.os !== 'Windows' && device.os !== 'MacOs') {
    ...
    function videoEvent(videoItem) {
      ...
      videoItem.addEventListener('timeupdate', () => {
        ...
        analytics.progress = video.currentDuration
      ...
      videoItem.addEventListener('progress', () => {
        ...
        if (duration > 0) {
          for (let i = 0; i < video.buffered.length; i++) {
            if (
              videoItem.buffered.start(videoItem.buffered.length - 1 - i) < videoItem.currentTime
            ) {
              progressBuffer.style.width = `${
                (videoItem.buffered.end(videoItem.buffered.length - 1 - i) * 100) / duration
              }%`
              break
            }
          }
      ...
      if (fileExtension == 'm3u8' || fileExtension == 'ts') {
        ...
        function hlsPlayer() {
          if (video.canPlayType('application/vnd.apple.mpegurl')) {
            ...
            loadNativeTracks()
          } else if (!video.canPlayType('application/vnd.apple.mpegurl') && Hls.isSupported()) {
            var hls = new Hls({
              debug: false,
            })
            if (encrypt == true) {
            ...
            let hlsInterval = setInterval(() => {
              hlsIntervalTime++
              if (hlsIntervalTime % 15 === 0 && hlsError && playerName == 'hls') {
            ...
            function setHLSBandwidth(band) {
              if (band != undefined && band != '') {
                ...
                let newBand
                band <= 999999 ? (newBand = stBand.substr(0, 3)) : (newBand = stBand.substr(0, 4))
                return newBand
              } else {
                return 360
              }
            ...
            function hlsQuality() {
              ...
              if (hlsData != undefined) {
                hlsItem += `<absdiv data-id='0' class='abs_quality ${
                  activeQuality == 0 || activeQuality == undefined ? 'abs_active' : ''
                ...
                hlsData.levels.forEach((item, index) => {
                  ...
                  const items = `<absdiv data-id='${index + 1}' class='abs_quality ${
                ...
                function dashQuality() {
                  ...
                  if (videoLevels != undefined && Array.isArray(videoLevels)) {
                    ...
                    videoLevels.forEach((item, index) => {
                      const items = `<absdiv data-id='${index + 2}' class='abs_quality ${
                ...
                function dashTrack() {
                  ...
                  if (dashAudios.length != undefined && Array.isArray(dashAudios)) {
                    ...
                    dashAudios.forEach((item, index) => {
                      const items = `<absdiv data-id='${index + 1}' class='abs_audio ${
                        ...
                        item.lang + ' ' + setDashSBandwidth(item.bitrateList[0].bandwidth)
                      }</p><p style='font-size:12px;font-family:sans-serif;margin:0 !important;'>${
                        def.icons.rightArrow
                      }</p></absdiv>`
                      dashAudio += items
            ...
            function getVmQuality() {
              ...
              vmPlayer
                ...
                .then(function (qualities) {
                  qualities.forEach((item, index) => {
                    vmqualityLabel += `<absdiv data-id='${item.id}' class='vmQuality ${
              ...
              vmPlayer
                ...
                .then(function (tracks) {
                  ...
                  tracks.forEach((item, index) => {
                    const itemSub = `<absdiv data-id='${item.language}' class='abs_subtitle ${
            ...
            function buffered() {
              vmPlayer
                ...
                .then(function (buffered) {
                  if (playerName == 'vimeo') {
                    progressBuffer.style.width =
                      parseInt(buffered[0][1] * progress.offsetWidth) / vmDuration +
                      buffered[0][1] +
                      'px'
                  }
                })
        ...
        if (premium.vast && premium.vast != '') {
          ...
          function loadAds() {
            ...
            function setupAds() {
              ...
              function playAds() {
                if (isVast == true) {
                  isPlaying !== false ? forcePause : true;
                  videoContent.load()
                  ...
                  if(adsType == 'linear'){
                    ...
                    adsContainer.style.background = 'transparent';
                    let nonT = 0;
                    const nonLinearTimer = setInterval(() => {
                      nonT++;
                      if(nonT >= 5){
                        clearInterval(nonLinearTimer);
                        isActiveAdd = false;
                        contentRelease();
                        nonT = 0;
                      }
              ...
              function onAdEvent(adEvent) {
                ...
                if (adEvent.type == 'complete' || adEvent.type == 'allAdsCompleted') {
                  contentRelease();
                  if (ad.isLinear()) {
          ...
          function xmlVast() {
                    ...
                    if (allAds) {
                      allAds.forEach((item, index) => {
                        let time = xmlLinkparser(
                          item.querySelector('Duration')?.textContent ?? undefined,
                        )
                        let vastAds = xmlLinkparser(
                          item.querySelector('MediaFile')?.innerHTML ?? undefined,
                        )
                        let vastAdsType =
                          item.querySelector('MediaFile')?.getAttribute('type') ?? undefined
                        let impressions = xmlLinkparser(
                          adsXml.querySelector('Impression')?.innerHTML ?? undefined,
                        )
                        let Plays = xmlLinkparser(
                          adsXml.querySelector('Plays')?.innerHTML ?? undefined,
                        )
                        let Clicks = xmlLinkparser(
                          adsXml.querySelector('ClickTracking')?.innerHTML ?? undefined,
                        )
                        let vastImg = xmlLinkparser(
                          item.querySelector('ClickTracking')?.innerHTML ?? undefined,
                        )
                        let vastReffer = xmlLinkparser(
                          item.querySelector('ClickThrough')?.innerHTML ?? undefined,
                        )
                        vastStart(
                          vastAds,
                          vastAdsType,
            ...
            function vastStart(ads, type, url, img, duration, clicks, Plays, impressions) {
              if (ads != undefined) {
                def.fn.attribute(adsContentVideo, {
                  src: ads,
                  type: type,
                ...
                adsContentVideo.addEventListener('timeupdate', () => {
                  ...
                  if (adsCurrentTime > 10) {
                    ...
                    def.fn.html(
                      adSkip,
                      `Skip in ${(adsTime < 10 ? 10 : adsTime) - adsCurrentTime}s `,
                    )
                    vastImpressions(impressions)
                  } else {
                    adSkip.style.display = 'none'
                    adsTimer.style.display = 'flex'
                    def.fn.html(adsTimer, (adsTime < 10 ? 10 : adsTime) - adsCurrentTime)
                  }
                })
              }
            }
    ...
    function loadCaptions(subtitleTracks) {
      hlsSubtitle = ''
      def.fn.html(settingsOptionAfter, '')
      if (subtitleTracks != undefined && subtitleTracks.length > 0) {
    ...
    function toggleScrubbing(e) {
      if (!isLive) {
        ...
        if (
          ...
          analytics.progress = (percent * (video.duration ?? 0)) / progress.offsetWidth
        } else if (playerName == 'youtube') {
          youtubePlayer.seekTo(
            (percent * (currentEvent.duration ?? 0)) / progress.offsetWidth,
            true,
          )
          analytics.progress = (percent * (currentEvent.duration ?? 0)) / progress.offsetWidth
        } else if (playerName == 'vimeo') {
          vmPlayer.setCurrentTime((percent * (vmDuration ?? 0)) / progress.offsetWidth)
          analytics.progress = (percent * (vmDuration ?? 0)) / progress.offsetWidth
        }
      }
      events('progress', analytics.progress)
      def.fn.postAnalytics(analytics)
    }
    ...
    function getFullDuration() {
      ...
      events('duration', maxDuration?.toFixed(2))
    }
    ...
    function getCurrentDuration() {
      if (
        ...
        var size = parseInt((video.currentTime * progress.offsetWidth) / video.duration)
    ...
    function getBuffer() {
      ...
      if (duration > 0 && playerName == 'youtube') {
        progressBuffer.style.width =
          parseInt(currentEvent.currentTime * progress.offsetWidth) / currentEvent.duration +
          currentEvent.buffer +
          'px'
      }
    }
    ...
    function seekBackward() {
      if (
        ...
        if (video.currentTime - 15 >= 0) {
          ...
          var size = parseInt(((video.currentTime - 15) * progress.offsetWidth) / video.duration)
      ...
      if (playerName == 'youtube') {
        if (currentEvent.currentTime - 15 >= 0) {
          ...
          var size = parseInt((0 * progress.offsetWidth) / currentEvent.duration)
    ...
    function seekForward() {
      if (
        ...
        if (video.currentTime + 15 <= video.duration) {
          ...
          var size = parseInt((video.duration * progress.offsetWidth) / video.duration)
    ...
    function APIYoutube(fileURL) {
      ...
      if (fileURI.indexOf('youtube.com') > -1 || fileURI.indexOf('youtu.be') > -1) {
        const regex =
          /(?:https?:)?(?:\/\/)?(?:[0-9A-Z-]+\.)?(?:youtu\.be\/|youtube(?:-nocookie)?\.com\/\S*?[^\w\s-])((?!videoseries)[\w-]{11})(?=[^\w-]|$)(?![?=&+%\w.-]*(?:['"][^<>]*>|<\/a>))[?=&+%\w.-]*/
    ...
    function rightOptions(item, container) {
      ...
      class ContextMenu {
        ...
        createItemMarkup(data) {
          ...
          if (data.adsDivider) item.setAttribute('data-adsDivider', data.adsDivider)
      ...
      const menuItems = [
        ...
        {
          content: `${def.fn.getLang('version')} 1.1.1`,
          adsDivider: 'top',
        },
      ]
