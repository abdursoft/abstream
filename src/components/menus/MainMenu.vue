<template>
  ...
  <div class="flex items-center justify-between md:justify-start gap-2 md:gap-5 h-[48px] fixed z-[9] top-0 w-full p-3 md:p-5"
    ...
    <div class="flex items-center justify-between md:justify-start gap-2 md:gap-5 w-[40] md:w-2/4">
      ...
      <img :src="siteData?.mobile_logo" :alt="siteData?.site_title" class="w-[50px] h-[31px] md:hidden">
      <!-- Logo section end  -->

      <!-- menu nav links start  -->
      <div class="hidden md:flex items-center justify-end gap-3 text-white uppercase">
        <router-link :class="{ 'text-red-500': $route.name === 'home' }" to="/home">{{ $t('menu.home') }}</router-link>
        <router-link :class="{ 'text-red-500': $route.name === 'liveTv' }" to="/live-tv">{{ $t('menu.tv') }}</router-link>
        <router-link :class="{ 'text-red-500': $route.name === 'subscribe' }" :to="{ name: 'subscribe' }">{{
          $t('menu.subscribe') }}</router-link>
        <router-link :class="{ 'text-red-500': $route.name === 'tvShows' }" to="/tv-shows">{{ $t('menu.tvShow') }}</router-link>
      </div>
      <!-- menu nav links end  -->
    </div>
